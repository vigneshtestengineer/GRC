...
TESSERACT_CMD = get_str("ocr", "tesseract_cmd", r"E:\Tesseract-OCR\tesseract.exe")
REPORTS_DIR = get_path("paths", "reports_dir", "reports")
OCR_TIME_BUDGET_SECONDS = 2.5
...
class GRCLoginPage(BasePage):
    ...
    def locate_and_convert_captcha(self) -> str:
        """
        Fast OCR flow:
        1) quick pass with light preprocessing and minimal OCR configs
        2) expensive fallback only if needed (and within time budget)
        """
        ...
        try:
            ...
            # Fast path first: fewer images/configs for lower latency.
            candidate_images = [
                ("quick", self._quick_prepare_captcha_image(image)),
            ...
            for image_name, candidate_image in candidate_images:
                for config in ocr_configs:
                    if perf_counter() - start_time > OCR_TIME_BUDGET_SECONDS:
                        break
                    cleaned_text, confidence = self._extract_text_with_confidence(candidate_image, config)
                    if not cleaned_text:
                        continue

                    # Character-segment OCR is expensive; run only when needed.
                    if not self._is_valid_captcha_text(cleaned_text):
                    ...
                    if self._is_valid_captcha_text(cleaned_text) and score > best_candidate["score"]:
                        ...
                        # Early return on high-confidence valid OCR for speed.
                        if confidence >= 65 and 4 <= len(cleaned_text) <= 8:
            ...
            # Fallback pass only when quick pass has no valid candidate.
            if not best_candidate["text"] and perf_counter() - start_time <= OCR_TIME_BUDGET_SECONDS:
                fallback_images = [
                ...
                for image_name, candidate_image in fallback_images:
                    if perf_counter() - start_time > OCR_TIME_BUDGET_SECONDS:
                        break
    ...
    def _quick_prepare_captcha_image(self, image):
    ...
    def _save_captcha_debug_image(self, image, name):
        debug_dir = os.path.join(REPORTS_DIR, "captcha_debug")
    ...
    def _is_invalid_captcha_displayed(self, timeout=3):
        """
        Check whether the Invalid Captcha validation message is visible.
        """
        end_time = datetime.now().timestamp() + timeout
        while datetime.now().timestamp() < end_time:
            for element in self.driver.find_elements(*self.INVALID_CAPTCHA_ERROR):
                try:
    ...
    def login(self, username, password, group_name, captcha_text=None):
        ...
        for attempt in range(1, max_captcha_attempts + 1):
            if current_captcha is None or not str(current_captcha).strip():
                current_captcha = self.get_captcha_text(popup_already_closed=True)

            self.logger.info(
                "Submitting login attempt %d/%d with captcha.",
                attempt,
                max_captcha_attempts,
            )
            self.enter_captcha(current_captcha)
            self.wait.until(
                lambda d: d.find_element(*self.CAPTCHA_TEXTBOX).get_attribute("value").strip() == current_captcha
            )
            self.sleep(1)
            ...
            if self._is_invalid_captcha_displayed(timeout=3):
                self.logger.warning(
                    "Invalid Captcha received on attempt %d/%d. Capturing a new captcha and retrying.",
                    attempt,
                    max_captcha_attempts,
                )
                if attempt == max_captcha_attempts:
                    raise RuntimeError(
                        f"Login failed after {max_captcha_attempts} attempts due to repeated Invalid Captcha."
                    )
                current_captcha = None
                continue

            self.sleep(1)  # wait for login to process, adjust as needed
